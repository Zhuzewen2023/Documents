...
* EKF：一个滤波器里同时估计姿态/偏置，acc/mag 都是观测；就算按顺序更新，它们仍在同一 $P$ 里耦合，mag 可能影响 roll/pitch。
...
$$
\dot{q}(t) = A(t)q(t), \quad A(t) = \frac{1}{2}\Omega(w(t))
$$
...
$$
\tilde{z} = \frac{z_{k1}}{\|z_{k1}\|}
$$


## Step1: 用四元数预测“期望重力方向”$h_1(\hat{q})$

“把导航系重力$\begin{bmatrix}0,0,1\end{bmatrix}$”旋到 body 系后的方向（单位向量形式）：
$$
h_1(\hat{q}) = \begin{bmatrix}2q_1q_3 - 2q_0q_2 \\ 2q_0q_1 + 2q_2q_3 \\ q_0^2 - q_1^2 -q_2^2 + q_3^2\end{bmatrix}
$$
...
归一化：
$$
\hat{q}_k \leftarrow \frac{\hat{q}_k}{\|\hat{q}_k\|}
$$
协方差更新：
$$
P_k = (I - KH_{k1})P^-_k
...
这里的$q_i$用的当前用于Stage2的先验$\hat{q}_k^{(1)}$里的分量。
...
等价于：不再做$P^- = APA^T + Q$这步，直接用一个“当前的 $P$”参与后面的增益计算。
